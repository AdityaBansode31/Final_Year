...
if(isset($_POST['submit']))
  {
    ...
    if ($query) {
        ...
        foreach($facilities as $f){
            ...
            $price = $_POST["adf_".$f."_price"]??0;
            $status = $_POST["adf_".$f]??"No";

            if($status=="Yes") $status = 1;
            else $status = 0;
            $query = "UPDATE `tblpgfacilities` SET `price` = '$price', status='$status'  WHERE pg_id ='$pg_id' AND name = '$f' ;";
...
<?php } ?>
                      ...
                      <div class="form-group has-success">
                        <label class="col-sm-3 control-label col-lg-3" for="inputSuccess" style="text-align: center; color: red">Additional Facilities</label>
                    ...
                    <?php
                        ...
                        foreach($facilities as $facility){  
                            // Fetch Deatials
                            $query = "SELECT * FROM tblpgfacilities WHERE pg_id = '$pg_id' and name = '$facility' ; ";
                    ...
                    ?>
                    <div class="form-group">
                        <label class="col-sm-3 control-label col-lg-3" for="inputSuccess"><?=$facility?></label>
                        <div class="col-lg-6">
                            <label class="checkbox-inline">
                                <input type="radio" id="adf_<?=$facility?>" name="adf_<?=$facility?>" value="Yes" required="true" <?=$status==1?"checked":""?>> Yes
                            </label>
                            <label class="checkbox-inline">
                                <input type="radio" id="adf_<?=$facility?>" name="adf_<?=$facility?>" value="No" <?=$status==0?"checked":""?>> No
                            </label>

                            <label class="input-inline">
                                <input type="number" id="adf_<?=$facility?>_price" name="adf_<?=$facility?>_price" value="<?=$price?>">
                            </label>
